...
2026-09-08：切块及上下文恢复逻辑经用户授权由助手验收通过；索引已按最新笔记重建，82 篇、873 块，33 项测试通过。现有 7 题完成真实检索验证：4 道正例命中预期笔记，2 道负例正确过滤，1 道原题库已知弱项仍未命中。新方案按结构切块（目标 500、正文上限 800，表格单行可例外），跳过顶部属性，表格按行保护，命中后恢复完整原始段落/列表/表格并去重。连续编号中间的表格与步骤共同恢复。当前索引为新版本，旧索引备份保留；尚未进入答案生成，历史验收进度保留如下。
