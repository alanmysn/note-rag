...
2026-09-08：切块及上下文恢复逻辑经用户授权由助手验收通过（32 项测试、本地分词器容量检查通过）；索引待确认重建，真实检索效果待验证。新方案按结构切块（目标 500、正文上限 800，表格单行可例外），跳过顶部属性，表格按行保护，命中后恢复完整原始段落/列表/表格并去重。连续编号中间的表格与步骤共同恢复。旧索引不兼容，新版检索会提示先重建；历史验收进度保留如下。
